...
**面向游戏、叙事与影像创作的本地知识组织及 LLM 协作工作台。**
...
**当前源码版本：0.9.0-rc.3 · 项目格式：1 / 2 · 接口协议：1**
...
> **0.9.0-rc.3 为待用户手动验收的开发版本。** 示例库只读浏览，明确创建练习副本后才写入本地；预设是正式新项目的工作起点。工程检查与真实设备／模型验收分开记录。详见 [任务状态](docs/tasks/STATUS_0.9.0.md) 与 [本轮验证](docs/protocol/QA_0.9.0_RC3.md)。

## 0.9 候选的新工作流程

**共享模块 → Quest 多轮问答 → 人工审核提案 → 素材任务与候选回导 → 二维有声排演 → 再次提问。**

预设只提供起步内容。全部格式2项目均可组合地图、角色、地点、剧情、场次、台词、分镜、声音和排演；游戏DD可以插入分镜，剧本可以引用地图，不按项目预设限制模块。地图源与战斗/剧情/镜头覆盖层分离，修改本处标注不会复制或污染共享源。

创作模块、Quest和排演有独立工作区，文档中可显示问题层。六类预设（空白、游戏、叙事、剧本、微电影、综合）仅建立起步结构。统一示例库提供基础示例和五类跨领域示例的只读浏览；明确创建独立练习副本后才写入示例内容。综合样例提供60份文档、74个对象、18份媒体和78秒排演；示意图、合成演示音和预置问答不是真实用户决定。

图片/配音生产包使用稳定任务身份和来源哈希。回导不等于采用，旧依据的结果不会自动覆盖当前镜头；桌面候选提供受控ComfyUI适配，但真实模型工作流仍需验收。排演可导出自包含HTML，不是完整剪辑软件或MP4渲染器。

新项目使用格式2，媒体在`assets/objects/<sha256>/`不可变存放，由历史清单引用。旧格式1项目保持读取，启用新能力前在用户指定副本升级，不原地改写旧历史。完整协议见 [创作模块](docs/protocol/CREATIVE_0.9.md)、[Quest](docs/protocol/QUEST_0.9.md) 和 [媒体/生产](docs/protocol/MEDIA_0.9.md)。
...
1. 在项目库选择“浏览示例”学习；浏览不会创建项目或文件。需要动手时，明确创建独立练习副本。
2. 正式工作从“新建项目”选择空白、基础 GDD 或跨领域预设，再指定保存目录。
3. 编写总纲和专项设计，按需要建立分类、引用与问询；通过“与 LLM 协作”生成本轮开场白，完成修改后核对正式文档和版本记录。

网页版明确创建的虚构练习副本与自动恢复点保存在浏览器专用本机存储中，清除网站数据会影响这些内容；需长期保留时请导出到本机文件夹。只读浏览不建立副本，正式项目必须选择磁盘目录。文件权限、两端接续与恢复边界见[双版本使用说明](docs/双版本开发与发布.md)。
...
├── assets/objects/     格式2的不可变图片与声音（以内容哈希定位）
├── versions/           公开快照、媒体引用、版本清单与基线
...
当前候选 MCP 注册 **32 个工具**。保留原20个工具，并新增能力发现、对象读取、Quest多轮接续、生产预览和图片/音频读取；还提供模块Schema资源与续轮Prompt。除项目列表、搜索、上下文、历史、问询、提案、导入预检与外部批次外，还提供当前协作状态、单文档分页读取、项目级提案状态、图片附件读取、命名基线、备份状态，以及恢复/撤销的只读计划。MCP 是受审核的策划协作接口，目标是覆盖 LLM 实际需要的策划业务，而不是复制所有界面按钮。
...
0.9.0-rc.3 的应用源码已提交到功能分支，最终发布状态以 GitHub 上的 PR、Pages 部署和 Release 附件为准。`main` 为共享主线，任务分支使用 `codex/` 前缀。开发协作要求见 [AGENTS.md](AGENTS.md)；修改业务行为时应同步维护协议与验收记录。仓库默认不运行自动化测试套件，按任务执行必要的类型检查、构建和人工验收，并明确记录未覆盖项。
...
当前面向本地单用户使用，采用线性完整快照；尚不包含云同步、多人权限、历史分支合并、自动调用模型或自动更新。传统文档导入单包上限为 150 MB、10,000 个文件；新媒体、生产和排演包边界见媒体协议；与后续内容重叠的撤销可能需要人工合并。桌面包尚未代码签名，独立网页版未提供离线 PWA 安装。
