...
当前 MCP 注册 **12 个工具**，覆盖项目列表、搜索与上下文、历史读取、问询发布、提案暂存、目录导入预检，以及外部编辑批次和封版。MCP 是受审核的策划协作接口，**不等同于开放全部应用功能**。

公开 Markdown、对白与色板设计块，以及受控的公开排版和笔画 JSON，可以通过通用文本提案提交候选，再由用户审核。项目创建与打开、现有附件的直接读取和上传、提案列表与状态查询、导出和基线管理等，尚无对应的完整 MCP 工具入口。完整覆盖情况、上下文缺口和建议优先级见 [MCP 能力审计](docs/protocol/MCP_CAPABILITY_AUDIT.md)。
