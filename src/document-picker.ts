...
export function pickDocument(snapshot:ProjectSnapshot,options:{title?:string;exclude?:string|string[];external?:boolean}={}):Promise<KnowledgeNode|string|null>{
  return new Promise(resolve=>{
    ...
    const render=()=>{
      const query=input.value.trim(),items=snapshot.nodes.filter(node=>{
        if(node.kind==='system'||(Array.isArray(options.exclude)?options.exclude:[options.exclude]).includes(node.id)||node.status==='archived')return false;
        const source=snapshot.documents.find(doc=>doc.id===node.documentId);let aliases:string[]=[];
        if(source)try{aliases=documentAliases(readHeader(source.text).metadata.aliases);}catch{/* 损坏的头部不影响标题检索。 */}
        return `${node.title} ${snapshot.groups.find(group=>group.id===node.group)?.label??''} ${aliases.join(' ')}`.toLocaleLowerCase().includes(query.toLocaleLowerCase());
      }).slice(0,60);
