...
export class GraphInteraction {
  ...
  constructor(private surface:GraphSurface){
    ...
    surface.container.append(this.overlay,this.preview,this.feedback);
  ...
  selectAll(){this.multi=new Set(this.surface.nodes().filter(n=>n.visible&&n.data.kind==='document'&&n.data.documentType!=='gdd').map(n=>n.data.id));this.update();}
  selectedIds(){return this.multi.size?[...this.multi]:this.surface.selected()?[this.surface.selected()!]:[];}
  private screen(point:THREE.Vector3){const p=point.clone().project(this.surface.camera),box=this.surface.container.getBoundingClientRect();return{x:(p.x+1)*box.width/2,y:(1-p.y)*box.height/2};}
  ...
  private hit(x:number,y:number,exclude?:string){
    const box=this.surface.container.getBoundingClientRect();
    return this.surface.nodes().filter(n=>n.visible&&n.data.id!==exclude).map(n=>{const r=n.label.getBoundingClientRect(),p=this.screen(n.point);return{n,d:Math.hypot(p.x+box.left-x,p.y+box.top-y),inside:!n.label.hidden&&x>=r.left-8&&x<=r.right+8&&y>=r.top-8&&y<=r.bottom+8};}).filter(item=>item.inside||item.d<18).sort((a,b)=>Number(b.inside)-Number(a.inside)||a.d-b.d)[0]?.n;
  }
  private notice(text:string){this.feedback.textContent=text;this.feedback.hidden=!text;}
  private click=(event:MouseEvent)=>{if(this.blockClick){event.preventDefault();event.stopImmediatePropagation();this.blockClick=false;}else if(event.shiftKey){const id=(event.target as HTMLElement).closest<HTMLElement>('[data-graph-node]')?.dataset.graphNode;if(id){event.preventDefault();event.stopImmediatePropagation();this.multi.has(id)?this.multi.delete(id):this.multi.add(id);this.update();}}};
  private down=(event:PointerEvent)=>{
    ...
    if(this.surface.mode()==='galaxy'&&id===this.surface.core()){this.notice('总纲固定在星系中心');return;}
    ...
    const moving=new Set(this.multi.has(id!)?this.multi:[id!]);
  ...
  private move=(event:PointerEvent)=>{
    ...
    const target=hoverPort?this.surface.nodes().find(n=>n.data.id===hoverPort.dataset.node):this.hit(event.clientX,event.clientY,drag.id);drag.target=target?.data.id;drag.targetPort=hoverPort?.dataset.port;drag.wire=undefined;
    ...
    for(const n of this.surface.nodes())if(n.visible&&!n.pinned&&!drag.moving.has(n.data.id)&&n.data.id!==this.surface.core()&&n.data.kind!=='system'){
      for(const other of moved){const a=this.screen(n.point),b=this.screen(other.point),dx=a.x-b.x,dy=a.y-b.y;const distance=Math.hypot(dx,dy),radius=drag.mode==='network'?120:58;
        if(distance<radius){const offset=n.point.clone().sub(other.point);if(offset.lengthSq()<.1)offset.set(1,.5,0);if(drag.mode!=='galaxy')offset.z=0;n.point.add(offset.normalize().multiplyScalar((radius-distance)*.8));this.surface.move(n.data.id,n.point);}
      }
    }
  ...
  private up=(event:PointerEvent)=>{
    ...
    if(drag.mode!=='galaxy'){
      ...
      else if(!drag.port&&node?.kind==='system')edit={kind:'classify',ids:[...drag.moving].filter(id=>this.surface.nodes().find(n=>n.data.id===id)?.data.kind==='document'),group:node.id};
  ...
  private cancel=()=>{const drag=this.drag;if(!drag)return;this.drag=undefined;this.surface.restore(drag.before);this.surface.end();this.clean();};
  private keydown=(event:KeyboardEvent)=>{if(event.key==='Escape'&&this.drag){event.preventDefault();event.stopImmediatePropagation();this.cancel();}};
  /** 端口与相应卡片边缘同步投影，过渡中隐藏，避免端口先于卡片跳位。 */
  update(){
    const editing=this.enabled&&this.surface.mode()!=='galaxy',selected=this.surface.selected(),edge=this.surface.edge();
    ...
    if(signature!==this.signature){this.signature=signature;this.overlay.replaceChildren();
      ...
      else if(editing&&selected){const n=this.surface.nodes().find(n=>n.data.id===selected);if(n&&n.data.kind!=='system'){add(selected,'connect','拖出虚线，建立手工关联');if(n.data.kind==='document'&&n.data.documentType!=='gdd')add(selected,'classify','拖出实线，改变主要分类');}}
    }
